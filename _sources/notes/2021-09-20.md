...
We can also extract each of those statistics individually, by name.  The quantiles
...
Each one goes by the same name, mostly, but that also the Pandas documentation is the bes way to find more information on every pandas method.
```
