...
````{margin}
```{note}
You can read more about the [details of data types](https://pandas.pydata.org/docs/user_guide/basics.html#basics-dtypes) in Pandas
```
````
...
type(help_df_types)
```
a `Series` is like a dataframee, but just one row with headings, and then rotated.
...
1. use a list comprehension to create a list that you could use as column names for data that consists of `N` measurements. Set `N=5` for now, but you suspect that the number might change.
...
```
1. Create a lambda funcation to print return the first 2 rows of a data frame
